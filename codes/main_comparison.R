...
comp.file <- "inputs/postSEVILLA_v1.csv" # set location of the accelerator merge file!!!
output.label <- "_postSEVILLA" #all outputs will end end with that label (except plots to avoid duplicates)
output.folder <- "dec05" #subfolder name in outputs and plots folder for versioning
...
create.modelVSaglink <- FALSE #quite fast. compares trends between models and aglink for all time series where data in both is present
create.model.diverging.trend.ranking <- FALSE #quite fast. compares trends between models and reports them ranked by max trend divergence
  only.opposing.signs <- FALSE #This will cause reporting to only include time series where trends between lowest and highest model have opposite signs
  ...
  include.level.sheets <- FALSE #also provides sheets with level differences in 2020
